...
// INT8 with separate weight and input scales
...
int main(){
    printf("=== Suite #65c: INT8 Proper Quantization ===\n\n");
    int R=4096;
    ...
    // Separate quantization: per-room for weights, global for input
    float imax=0;
    for(int i=0;i<DIM;i++){float v=fabsf(__half2float(hi[i]));if(v>imax)imax=v;}
    float iscale=imax/127.0f;
    printf("  Input max: %.4f, iscale: %.6f\n", imax, iscale);

    std::vector<char> iq(DIM);
    for(int i=0;i<DIM;i++) iq[i]=(char)(int)((float)__half2float(hi[i])/iscale);

    std::vector<float> ws(R);
    std::vector<char> wq(R*DIM);
    for(int r=0;r<R;r++){
        float wmax=0;
        for(int d=0;d<DIM;d++){float v=fabsf(__half2float(hw[r*DIM+d]));if(v>wmax)wmax=v;}
        ws[r]=wmax/127.0f;
        float inv=ws[r]>0?127.0f/wmax:0;
        for(int d=0;d<DIM;d++) wq[r*DIM+d]=(char)(int)(__half2float(hw[r*DIM+d])*inv);
    }

    char*dwq,*diq; float*dws; float*doi8;
    ...
    float i8_us;
    {cudaEvent_t s1,e1; cudaEventCreate(&s1); cudaEventCreate(&e1);
     for(int i=0;i<WARMUP;i++) infer_i8<<<grid,256,0,s>>>(dwq,diq,doi8,dws,iscale,R,DIM);
     cudaStreamSynchronize(s); cudaEventRecord(s1,s);
     for(int i=0;i<ITERS;i++) infer_i8<<<grid,256,0,s>>>(dwq,diq,doi8,dws,iscale,R,DIM);
     cudaEventRecord(e1,s); cudaStreamSynchronize(s);
     float ms; cudaEventElapsedTime(&ms,s1,e1); cudaEventDestroy(s1); cudaEventDestroy(e1);
     i8_us=ms/ITERS*1000;
    }
    ...
    infer_i8<<<grid,256,0,s>>>(dwq,diq,doi8,dws,iscale,R,DIM);
    ...
    // Check quantized value ranges
    printf("\n  Quantized ranges:\n");
    int wmin=127,wmax=-128,imin=127,imx=-128;
    for(int i=0;i<DIM;i++){int v=(int)iq[i];if(v<imin)imin=v;if(v>imx)imx=v;}
    for(size_t i=0;i<(size_t)R*DIM;i++){int v=(int)wq[i];if(v<wmin)wmin=v;if(v>wmax)wmax=v;}
    printf("    Weights: [%d, %d]  Input: [%d, %d]\n", wmin,wmax,imin,imx);
